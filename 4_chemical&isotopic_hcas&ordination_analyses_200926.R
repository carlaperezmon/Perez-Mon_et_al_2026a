...
path_work="C:/Users/User/Desktop/who_cluster_stability/who_extra_analyses/"

setwd(path_work)

load("all_hcas_200926_hist.RData")
...
packages <- c("ggplot2","openxlsx","reshape2","dplyr",
              ...
              'gridExtra','patchwork','vegan','colorspace','fpc')
...
### LOAD ALL CHEMICAL AND ISOTOPIC DATA, ARRANGE TABLES AND SCALE FOR HCAS ------

# morphology 
data_morph=read.xlsx("who_morphology/allmorphology_forR.xlsx", sheet= "Sheet1") %>%
...
data_iso2 <- data_iso %>%  
  ...
  column_to_rownames(var = "lot_id3_rep") %>%
  dplyr::select(-lot_id3, -sample.id, -S34) 
...
outliers_hydro=c('W19_3') #remove outlier

data_hydro2 <- data_hydro %>%
  group_by(lot_id3_rep) %>%
  filter(!lot_id3_rep %in% outliers_hydro) %>% 
  column_to_rownames(var = "lot_id3_rep") %>%
...
mean(data_scaled_nir[,1])
sd(data_scaled_nir[,1])
...
data_scaled_icpms <- data_scaled_icpms[, colSums(!is.na(data_scaled_icpms)) > 0] #only include elements that were detected
data_scaled_icpms=data_scaled_icpms[!rownames(data_scaled_icpms) %in% c('W03_4'),] #remove outlier 
...
outliers_xrd=c('W18_2') #remove outlier
...
data_input=c('data_scaled_morph',
             'data_scaled_ftir',
             'data_scaled_iso',
             'data_scaled_hydro',
             'data_scaled_caco3',
...
directories_output=c('who_morphometry/hca_allmorphology',
                     'who_ftir/average',
                     'who_isotopes',
                     'who_isotopes/hydrogen',
                     'who_caco3',
                     'who_NIR',
                     'who_icpms',
                     'who_xrd')
...
cut_val_all=c(0.22,
              10.5,
              0.94,
              1.35, #1 for hydro if removing all outliers, 1.35 for minimum outliers
              0.8,
              0.38,
              9.3,
              58) #0.8 for caco3, 42 if leaving outlier for XRD
...
for (i in 1:length(data_input)) {
  ...
  #dendrogram plot with rectangles groups and bars ###
  ...
  # create pairwise distance matrix, ROC and plots
...
dir.create(file.path('stability_clusters_results'), showWarnings = TRUE)
...
for (i in 1:length(data_input)) {
  ...
  setwd('stability_clusters_results')
  
  dist.matrix=dist(data_scaled, method='euclidean')
  hclust_avg <- hclust(dist.matrix, method = 'average')
  
  cut_val=cut_val_all[i]
  hca_clusters <- cutree(hclust_avg, h = cut_val)
  
  #order cluster vector according to distance matrix
  ...
  for (j in 1:length(clusters_methods)) {
    ...
    sink(paste("cluster_stability_result_",data_input2[i],'_',clusters_methods[j],'.txt',sep=''), split = TRUE)
    ...
    write.xlsx(check_stability,paste("stability_groups_comparison_",data_input2[i],'_',clusters_methods[j],'.xlsx',sep=''), rowNames = TRUE)
  }
  ...
  write.xlsx(all_methods_stability,paste("stability_clust_methods_comparison_",data_input2[i],'.xlsx',sep=''),rowNames = TRUE)
...
### EXPLORING RELATIONSHIP BETWEEN TABLE MANUFACTURING DATES AND BETWEEN COLLECTION PLACES ----
